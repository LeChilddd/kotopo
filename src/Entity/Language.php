...
class Language
{
    ...
    #[ORM\OneToMany(mappedBy: 'language_id', targetEntity: UserLanguageLevel::class, orphanRemoval: true)]
    private $userLanguageLevels;

    #[ORM\OneToMany(mappedBy: 'language_id', targetEntity: Session::class, orphanRemoval: true)]
    private $sessions;
    ...
    public function addUserLanguageLevel(UserLanguageLevel $userLanguageLevel): self
    {
        if (!$this->userLanguageLevels->contains($userLanguageLevel)) {
            ...
            $userLanguageLevel->setLanguageId($this);
        }
    ...
    public function removeUserLanguageLevel(UserLanguageLevel $userLanguageLevel): self
    {
        if ($this->userLanguageLevels->removeElement($userLanguageLevel)) {
            ...
            if ($userLanguageLevel->getLanguageId() === $this) {
                $userLanguageLevel->setLanguageId(null);
            }
        }
    ...
    public function removeSession(Session $session): self
    {
        if ($this->sessions->removeElement($session)) {
            // set the owning side to null (unless already changed)
            if ($session->getLanguageId() === $this) {
                $session->setLanguageId(null);
            }
        }
